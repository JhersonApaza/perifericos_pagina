 ...
 * Migrado desde controllers/AuthController.js.
 * El método dashboard() ha sido extraído a features/dashboard/dashboard.controller.js.
...
class AuthController {
  ...
  async login(req, res) {
    ...
    res.render('auth/pages/login', { toast_warn, toast_ok });
  }
  ...
  async registro(req, res) {
      ...
      if (password !== confirm_password) {
        ...
        if (!pwdInfo.ok) {
          ...
          if (!vec) {
            ...
            // Verificar si este rostro ya está registrado en otra cuenta
            const usuariosConRostro = await this.modelo.listarConDescriptorFacial();
            let rostroDuplicado = false;
            for (const fila of usuariosConRostro) {
              const almacenado = this.normalizarDescriptor(fila.face_descriptor);
              if (!almacenado) continue;
              const dist = this.distanciaDescriptores(vec, almacenado);
              if (dist !== null && dist <= UMBRAL_ROSTRO) {
                rostroDuplicado = true;
                break;
              }
            }

            if (rostroDuplicado) {
              req.session.toast_warn = 'Este rostro ya está registrado en otra cuenta.';
            } else {
    ...
    delete req.session.toast_warn;
    delete req.session.toast_ok;
    res.render('auth/pages/registro', { toast_warn, toast_ok });
  }
  ...
  async verificarOtp(req, res) {
    ...
    delete req.session.toast_warn;
    delete req.session.toast_ok;
    res.render('auth/pages/verificar_otp', { toast_warn, toast_ok });
  }
